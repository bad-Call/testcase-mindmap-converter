...
为了确保数据转换的灵活性和可维护性，所有需要映射的字段（如优先级、用例类型、自动化状态、标签、模块类型）都将通过外部配置文件 `config.yaml` 进行管理，避免硬编码。
...
*   **`main.js`**：主入口文件，负责协调整个转换流程。它根据输入类型（JSON/MD）和输出类型（JSON/MD），调用相应的解析器和渲染器。同时，它处理剪切板和文件I/O的抽象层，并负责BOM的添加和移除。
...
*   **`config.js`**：负责加载和解析 `config.yaml` 配置文件，提供数据映射和关键字常量。
*   **`helpers.js`**：提供辅助函数，如BOM的添加和移除。
